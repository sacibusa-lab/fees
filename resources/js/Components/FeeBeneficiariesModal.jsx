...
const FeeBeneficiariesModal = ({ fee, onClose, bankAccounts = [] }) => {
    ...
    // You can allocate less than or equal to the total amount, remainder stays in main bucket.
    const isTotalValid = totalAmount <= fee.raw_amount;

    const handleSubmit = (e) => {
        e.preventDefault();
        if (!isTotalValid) {
            alert(`Total amount cannot exceed ₦${fee.raw_amount.toLocaleString()}`);
            return;
        }
                    ...
                    {!isTotalValid && (
                        <div style={{
                            ...
                            <div>
                                ...
                                <p style={{ margin: 0 }}>
                                    Total split cannot exceed ₦{fee.raw_amount.toLocaleString()}.
                                    Current: ₦{totalAmount.toLocaleString()} (Exceeds by: ₦{(totalAmount - fee.raw_amount).toLocaleString()})
                                </p>
                            </div>
                        </div>
                    )}
