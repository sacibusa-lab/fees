...
import { X, ArrowLeft, Search, Grid, List, Download } from 'lucide-react';
...
const ClassPaymentDetailModal = ({ isOpen, onClose, classData, feeId }) => {
    ...
    const handleExport = (type) => {
        ...
        if (type !== 'all') {
            studentsToExport = students.filter(s => s.status === type);
        }
        ...
        // Generate XLS (Simple HTML Table Format)
        const fileName = `${(metadata.className || 'Class').replace(/\s+/g, '_')}_${type}_Payment_Status.xls`;
        const headers = ['Name', 'Registration No.', 'Amount', 'Status'];
        ...
        let html = `
            ...
            <body>
                <table>
                    <thead>
                        <tr>${headers.map(h => `<th style="background-color: #f3f4f6; font-weight: bold; border: 1px solid #d1d5db;">${h}</th>`).join('')}</tr>
                    </thead>
                    <tbody>
                        ${studentsToExport.map(s => `
                            <tr>
                                <td style="border: 1px solid #d1d5db;">${s.name}</td>
                                <td style="border: 1px solid #d1d5db;">${s.reg_no}</td>
                                <td style="border: 1px solid #d1d5db;">${(s.status === 'completed' || s.status === 'paid')
                ? '0'
                : Math.max(0, (classData?.flatAmount || 0) - (s.discount || 0) - (s.received_amount || 0)).toLocaleString()
            }</td>
                                <td style="border: 1px solid #d1d5db;">${(s.status || 'Pending').charAt(0).toUpperCase() + (s.status || 'Pending').slice(1)}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
                    ...
                    <div className="header-actions-row">
                        ...
                        <div className="main-actions-group">
                            <div className="export-dropdown-container">
                                <button
                                    className="action-outline-btn export"
                                ...
                                {isExportDropdownOpen && (
                                    <div className="export-dropdown-menu">
                                        <button className="export-dropdown-item" onClick={() => handleExport('all')}>All</button>
                                        <button className="export-dropdown-item" onClick={() => handleExport('paid')}>Paid</button>
                                        <button className="export-dropdown-item" onClick={() => handleExport('completed')}>Completed</button>
                                        <button className="export-dropdown-item" onClick={() => handleExport('partial')}>Part Payment</button>
                                        <button className="export-dropdown-item" onClick={() => handleExport('pending')}>Pending</button>
                                    </div>
                                )}
                            </div>
                        </div>
