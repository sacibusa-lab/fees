...
class AdminCareController extends Controller
{
    ...
    {
        $roles = Role::withCount('users')->orderBy('created_at', 'desc')->get()->map(function ($role) {
            return [
                'id' => $role->id,
                'name' => $role->name,
                'slug' => $role->slug,
    ...
    public function updatePermissions(Request $request)
    {
        ...
        // We need to group by roleId first
        $rolePermissions = [];
        $roleSuperFlags = [];

        foreach ($matrix as $key => $isEnabled) {
            if (strpos($key, '_') !== false) {
                [$roleId, $param] = explode('_', $key);
                
                if ($param === 'super') {
                    $roleSuperFlags[$roleId] = (bool)$isEnabled;
                } else {
                    $permissionId = $param;
        ...
        $allRoleIds = array_unique(array_merge(array_keys($rolePermissions), array_keys($roleSuperFlags)));
        
        foreach ($allRoleIds as $roleId) {
            $role = Role::find($roleId);
            if ($role) {
                // Update super admin flag if provided
                if (isset($roleSuperFlags[$roleId])) {
                    $role->update(['is_super_admin' => $roleSuperFlags[$roleId]]);
                }
                
                // Sync permissions if provided (or if the role exists and was in the matrix)
                if (isset($rolePermissions[$roleId])) {
                    $role->permissions()->sync($rolePermissions[$roleId]);
                }
            }
        }
