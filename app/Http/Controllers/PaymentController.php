...
class PaymentController extends Controller
{
    public function overview(Request $request)
    {
        ...
        // Determine Term (Normalize) — default to current session's active term
        if (!$term) {
            $term = $currentSession->current_term ?? '1st Term';
        }
        ...
        $totalReceivedQuery->whereRaw("JSON_UNQUOTE(JSON_EXTRACT(metadata, '$.session_id')) = ?", [(string)$currentSession->id])
                           ->where('metadata->term', $term);
    ...
    public function transactions(Request $request)
    {
        ...
        $query = Transaction::where('institution_id', $institutionId)
            ->with(['student', 'fee']);
        ...
        DB::transaction(function() use ($notices, $institutionId, $validated, $feeId, $paymentMode, $customAmount, $description, $session, $term) {
            foreach ($notices as $notice) {
                ...
                if ($paymentMode === 'partial' && $customAmount) {
                     $remainingToPay = (float) $customAmount;
                } else {
                     $remainingToPay = $notice['total_due'];
                }

                if ($remainingToPay <= 0) continue;

                // Create separate transactions for each fee to ensure correct splitting
                foreach ($notice['fees'] as $feeNotice) {
                    if ($remainingToPay <= 0) break;

                    // Skip adjustments for splits (or handle them differently if they have beneficiaries)
                    // Currently, adjustments are tagged with 'is_adjustment' => true
                    $feeId = (isset($feeNotice['is_adjustment']) && $feeNotice['is_adjustment']) ? null : $feeNotice['id'];
                    
                    // Limit payment to the amount due for this specific fee
                    $paymentForThisFee = min($remainingToPay, (float) $feeNotice['amount']);
                    
                    if ($paymentForThisFee <= 0) continue;
                    ...
                    Transaction::create([
                        ...
                        'metadata' => [
                            ...
                            'fee_title' => $feeNotice['title']
                        ]
                    ]);

                    $remainingToPay -= $paymentForThisFee;
                }
                
                // If there's still remainder (e.g. overpayment or adjustments only), 
                // create a general transaction or apply to last fee
                if ($remainingToPay > 0) {
                     Transaction::create([
                        'institution_id' => $institutionId,
                        'student_id' => $notice['student']->id,
                        'fee_id' => null, // No specific fee for general remainder
                        'reference' => 'MAN-REM-' . strtoupper(Str::random(10)),
    ...
    public function exportSchedule(Request $request)
    {
        ...
        $callback = function() use ($notices) {
            ...
            fputcsv($file, ['S/N', 'Student Name', 'Admission Number', 'Class', 'Subclass', 'Total Fee (N)', 'Amount Paid (N)', 'Balance Due (N)']);
            ...
            foreach ($notices as $index => $notice) {
                fputcsv($file, [
                    ...
                    number_format($notice['original_total'] ?? $notice['total_due'], 2),
                    number_format($notice['total_paid'] ?? 0, 2),
